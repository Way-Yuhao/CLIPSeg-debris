...
from src.utils.yolo_utils.util import make_anchors
...
def get_yolo(variant: str, num_classes: int = 80) -> YOLO:
    """
    Factory function to create a YOLO model based on the specified variant.

    :param variant: The variant of YOLO to create ('n', 's', 'm', 'l', 'x').
    :param num_classes: The number of classes for the model.
    :return: An instance of the YOLO model.
    """
    if variant == "n":
        return yolo_v8_n(num_classes)
    elif variant == "s":
        return yolo_v8_s(num_classes)
    elif variant == "m":
        return yolo_v8_m(num_classes)
    elif variant == "l":
        return yolo_v8_l(num_classes)
    elif variant == "x":
        return yolo_v8_x(num_classes)
    else:
